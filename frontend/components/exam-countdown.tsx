...
export function ExamCountdown({
  ...
  const urgencyBgColors = {
    ...
    passed: 'bg-gray-50 dark:bg-gray-900/20 border-gray-200 dark:border-gray-800'
  }
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="text-2xl">{urgencyIcon[timeRemaining.urgency]}</span>
        ...
        <div className="flex flex-col items-center gap-1 ml-4">
          <div className={`
            ...
            ${timeRemaining.urgency === 'critical' ? 'bg-red-500 animate-pulse' : 
              timeRemaining.urgency === 'high' ? 'bg-orange-500' :
              timeRemaining.urgency === 'medium' ? 'bg-yellow-500' :
              timeRemaining.urgency === 'low' ? 'bg-green-500' :
              'bg-gray-400'}
          `} />
          <span className="text-xs text-gray-500 capitalize">{timeRemaining.urgency}</span>
        </div>
      </div>
      ...
      {timeRemaining.days > 0 && timeRemaining.urgency !== 'critical' && (
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-4 gap-2 text-center text-sm">
            <div>
              <div className="font-bold text-gray-900 dark:text-gray-100">{timeRemaining.days}</div>
              <div className="text-xs text-gray-500">days</div>
            </div>
            <div>
              <div className="font-bold text-gray-900 dark:text-gray-100">{timeRemaining.hours}</div>
              <div className="text-xs text-gray-500">hours</div>
            </div>
            <div>
              <div className="font-bold text-gray-900 dark:text-gray-100">{timeRemaining.minutes}</div>
              <div className="text-xs text-gray-500">mins</div>
            </div>
            <div>
              <div className="font-bold text-gray-900 dark:text-gray-100">{timeRemaining.seconds}</div>
